...
int OpenGLShaderProgram::addShaderFromSourceFile(ShaderType type, const char*path) {
    ...
    if (!shaderSource) {
        std::cerr << "Unable to load " << name << " shader from \"" << path << "\"" << std::endl;
        return 1;
    }
    ...
    if (!compiled) {
        std::cerr << "Unable to compile " << name << " shader from \"" << path << "\"" << std::endl;
        GLint maxLength = 0;
    ...
    delete[] shaderSource;
    std::cout << "Loaded " << name << " shader from \"" << path << "\"" << std::endl;
